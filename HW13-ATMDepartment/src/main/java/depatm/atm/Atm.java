...
public class Atm {
    private Cell cells[];
    private String name;
    private long balanceinternal=0;

    public Atm(Cell[] cellsLoad) throws Exception {
        installCells(cellsLoad);
    }

    public void setAtmName(String name) {
        this.name=name;
    }
    ...
    public void loadMoney(Cell[] cellsLoad) throws Exception {
        installCells(cellsLoad);
    }
    ...
    private void calculateBalance() {
        balanceinternal=0;
        for (int i = 0; i < cells.length; i++) {
            balanceinternal += this.cells[i].getNominal() * this.cells[i].getCount();
        }
    }
